...
Encoder.prototype._writeHeader = function (cb) {
  ...
  if (r !== 0) return cb(new VorbisError(r, 'vorbis_encode_init_vbr'));
  ...
  if (r !== 0) return cb(new VorbisError(r));
  r = binding.vorbis_block_init(this.vd, this.vb);
  debug('vorbis_block_init() return = %d', r);
  if (r !== 0) return cb(new VorbisError(r, 'vorbis_block_init'));
  ...
  if (r !== 0) return cb(new VorbisError(r, 'vorbis_analysis_headerout'));
...
Encoder.prototype._writepcm = function (buf, cb) {
  ...
  binding.vorbis_analysis_write(this.vd, buf, channels, samples, function (rtn) {
    ...
    if (rtn !== 0) {
      // error code
      return cb(new VorbisError(rtn, 'vorbis_analysis_write'));
    }
...
Encoder.prototype._blockout = function (cb) {
  ...
  binding.vorbis_analysis_blockout(vd, vb, function (rtn) {
    ...
    if (rtn === 1) {
      ...
      // error code
      cb(new VorbisError(rtn, 'vorbis_analysis_blockout'));
    }
  });
...
Encoder.prototype._flushpacket = function (cb) {
  ...
  binding.vorbis_bitrate_flushpacket(this.vd, packet, function (rtn) {
    ...
    if (rtn === 1) {
      ...
      // error code
      cb(new VorbisError(rtn, 'vorbis_bitrate_flushpacket'));
    }
  });
};
...
Encoder.prototype._flush = function (cb) {
  ...
  function process () {
    ...
    if (r === 0) {
      ...
      // error code
      cb(new VorbisError(r, 'vorbis_analysis_eos'));
    }
  }
};
