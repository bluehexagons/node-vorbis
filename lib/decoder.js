...
Decoder.prototype._transform = function (packet, _, cb) {
  ...
  if (this._headerCount > 0) {
    ...
    if (r !== 0) {
      return cb(new VorbisError(r, 'vorbis_synthesis'));
    }

    // TODO: async...
    r = binding.vorbis_synthesis_blockin(vd, vb);
    if (r !== 0) {
      return cb(new VorbisError(r, 'vorbis_synthesis_blockin'));
    }

    pcmout(self.bitDepth);
  }

  function pcmout (bitDepth) {
    // TODO: async...
    if (!bitDepth) {
      bitDepth = 32;
    }
    var b = binding.vorbis_synthesis_pcmout(vd, channels, bitDepth);
    if (b === 0) {
      ...
      // some other error...
      cb(new VorbisError(b, 'vorbis_synthesis_pcmout'));
    } else {
...
Decoder.prototype._synthesis_init = function () {
  ...
  if (r !== 0) {
    return new VorbisError(r, 'vorbis_synthesis_init');
  }
  r = binding.vorbis_block_init(this.vd, this.vb);
  if (r !== 0) {
    return new VorbisError(r, 'vorbis_block_init');
  }
};
